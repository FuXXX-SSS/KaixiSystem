...
if(GetUrlParam("status")==1){
    table.render({
        elem: '#test'
        ,url: url + '/bloodInspect/queryBloodInspect'
        ,method:'post'
        ...
        ,cols: [[
            ...
            //,{field:'isaddTestresultType', title: '复测原因',align:'center',rowspan: 2}
...
table.on('tool(demo)', function(obj){
    ...
    if(obj.event === 'detail'){//添加检验结果
        ...
        window.location.href = "testResults.html?id=" + obj.data.id+"&status="+status;;
   ...
   getCheckLength: function(){ //批量删除
    ...
    allout: function () {//批量导出
       ...
       // param.admissionNumber = admissionNumber
        ...
        //param.testState = 0
        $.ajax({
            url: url+"/out/outBloodInspectAll",
            type: "POST",
            dataType: "JSON",
            json: false,
            mask: true,
...
$("#allinput").change(function (e) {
    ...
    $.ajax({
        url: "http://47.93.22.122:8104/SSM/bloodInspect/importOMATR",
        type: "POST",
        json: false,
        mask: true,
        beforeSend: function (XMLHttpRequest) {
...
$(".officeadd input[name='points']").keyup(function(){
...
$("#val1").blur(function(){
    ...
    var val1= $("#val1").val()
    if (isNaN(val1)){
        $(".layui-layer-btn0").css({"cursor":"not-allowed"});
