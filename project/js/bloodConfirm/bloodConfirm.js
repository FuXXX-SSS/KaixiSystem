...
$(function () {//下拉框
    ...
    // $(".laytable-cell-1-0-18 .layui-table-edit").placeholder("123")
...
table.render({
    ...
    , parseData: function (res) {
        if (res.code == 0) {
            ...
            data.rows.forEach(function (item,index) {
                ...
                if (item.sampleTemperature==null){
                    item.sampleTemperature=11
                }
            })
            if (data.total == 0) {
            ...
            http.ajax({
                ...
                if (data.code == 0) {
                    ...
                    if (name==0){
                        ...
                        $('tbody tr').find('td:eq(-3) div').append($obj)
                        form.render()
                    }
    ...
    , done: function (res) {
        var name = $("#check").val()
        if (name==1){
            $('tbody tr').find('td:eq(-5),td:eq(-4)').prop("disabled",true);
            form.render()
        }
    }
});
...
table.on('tool(demo)', function (obj) {
    ...
    if (obj.event === 'detail') {
        ...
        if (val1) {
            if (val2) {
                http.ajax({
                    ...
                    data: {
                        id: data.id,
                        sampleTemperature: val1,
                        sampleSize: val2,
                        bloodSamplePreparation: $('#select').val()
                    },
                }).then(function (data) {
                    if (data.code == 0) {
                        layer.msg('添加成功');
    ...
    } else if (obj.event === 'bloodVerify') {
        var id = data.id
        http.ajax({
            url: "/bloodVerify/getSample",
            type: "POST",
            json: false,
            mask: true,
            beforeSend: function (XMLHttpRequest) {
            ...
            if (data.code == 0) {
                if (data.data.bloodSamplePreparation) {
                    $("#bloodSamplePreparation").val(data.data.bloodSamplePreparation)
                }
                if (data.data.sampleSize) {
                    $("#sampleSize").val(data.data.sampleSize)
                }
                if (data.data.sampleTemperature) {
                    $("#sampleTemperature").val(data.data.sampleTemperature)
                }
            }
        }, function (err) {
            console.log(err);
        ...
        layer.open({
            ...
            yes: function (index, layero) {
                ...
                console.log(sampleTemperature);
                layer.close(index)
                http.ajax({
                    url: "/bloodVerify/updateSample",
                    type: "POST",
                    json: false,
                    mask: true,
                    beforeSend: function (XMLHttpRequest) {
...
var active = {
    ...
    addition: function () { //批量添加
        ...
        if (data.length == 0) {
            ...
            $(".result").parent(".layui-layer-content").css("height", "450px")
        }
    },
};
